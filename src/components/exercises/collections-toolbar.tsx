...
export interface CollectionsTableFilters {
  search: string;
}

interface CollectionsToolbarProps {
  filters: CollectionsTableFilters;
  onFiltersChange: (
    next:
      | CollectionsTableFilters
      | ((prev: CollectionsTableFilters) => CollectionsTableFilters),
  ) => void;
  onNew: () => void;
}

/** Search toolbar + primary "New collection" action. */
export function CollectionsToolbar({
  ...
  function update(patch: Partial<CollectionsTableFilters>) {
